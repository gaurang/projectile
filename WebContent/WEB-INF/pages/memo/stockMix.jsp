...
//	$( "#memoDate" ).val('${CURR_DATE}');
	...
	if('${USER_SESSION.currency}' != 'USD')
		getExRate('USD','${USER_SESSION.currency}',$('#exRate'));
});
...
//}
    ...
    $('#memoResult').setGridParam({url:'memoLoadGridMix.html?fromSrch='+frmId+'&'+$('#searchForm').serialize(), datatype:'json' ,postData:{ selectedPkts : null}}).trigger("reloadGrid");
...
<body>
	...
	<jsp:include page="../inc/inc_header.jsp">
		...
		<jsp:param name="subPage" value="mix" />
	</jsp:include>
	<div class="container">
		<form id="searchForm" name="searchForm">
			<input type="hidden" value="1" name="salesMix">
			<div class="content">
				<a class="trigger2" href="#">&nbsp;</a>
				<div class="panelBox2" id="leftFilter2">
					<table>
						<tr>
							<td><label for="partyStock" class="ttl_names">Party</label>
							</td>
							<td><select id="partyStock" name="partyStock" style="width: 110px;">
							...
							<td><span class="ttl_names" id="memoSpan">Memo Nos.</span>
							</td>
							<td><input type="text" id="memoNos" name="memoNos" onkeydown="if (event.keyCode == 13)submitSearch(this);" onfocus="$('#pktNos').val('');" /></td>
							<td></td>
							<td><label for="rootPktSrch" class="ttl_names">Rough(Root)</label>
							</td>
							<td><input type="checkbox" name="rootPktSrch" id="rootPktSrch" value="1" onchange="searchoption(this);" "/></td>
							<td><span class="ttl_names">Pkt Nos.</span></td>
							<td>
							     <input type="text" id="pktNos" name="pktNos" onkeydown="if (event.keyCode == 13)submitSearch(this);" onfocus="$('#memoNos').val('');" />
							</td>
							<td><input type="button" value="Search" id="btnSearch2" onclick="submitSearch(this);"></td>
						</tr>
			...
			<c:if test="${status.count%2 != 0}"><tr><td></c:if> --%>
						<c:choose>
							<c:when test="${pl.fieldDisplayType== 'MULSEL'}">
								<div class="row" id="${pl.prp}_row">
									<label for="${pl.prp}" title="${pl.prpDesc}">${pl.prpDesc}</label>
									<div class="element" style="width: 160px;">
										<select name="${pl.prp}" id="${pl.prp}" class="select" multiple="multiple">
			...
			<c:if test="${status.count%2 != 0}"></td></c:if> --%>
					...
					<!-- <label for="srchPair" style="white-space: nowrap;">Only Pairs</label> <input type="checkbox" name="srchPair" id="srchPair" value="1">-->
					<input type="button" value="reset" id="reset"
						onclick="javascript:clearForm('#leftFilter');"> <input
				...
				<table align="center" width="95%">
					<tr>
						...
						<td><input type="text" name="buyerName" id="buyerName" size=30 /> 
						    <input type="hidden" name="partyAccId" id="partyAccId" />
						    <a href="javascript:void(0);" onclick="addTab('New Party','partyActionController.html?typ=BYR');">Add</a> <br /> <a
							href="javascript:void(0);" onclick="partyRep();">Report</a>&nbsp;&nbsp;<a
							href="javascript:void(0);" onclick="deposit();">Deposit</a>
						</td>
						...
						<td><select name="term" id="term" onchange="submitSearch(document.getElementById('btnSearch2'));">
					...
					<tr>
						...
						<td>
							<input type="text" name="brokerName" id="brokerName" size="16" /> 
						...
						<td><input type="text" name="dueDate" id="dueDate" size="10" />
							Days<input type="text" name="days" id="days"
							onchange="calDate(this,'dueDate');" size="3" />
						</td>

						<td class="ttl_names">EX. Rate :</td>
						<td><input type="text" name="exRate" id="exRate" size=10 />
						</td>
						<td class="ttl_names"><label for="accType_L">Local :
						</label><input type="radio" name="accType" id="accType_L" value="L" /><label
							for="accType_E">Export : </label> <input type="radio"
							name="accType" id="accType_E" checked="checked" value="E" /></td>
