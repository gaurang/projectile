...
<div id="customerBalanceSheetBody" align="center">
    <form action="custBalanceSubmit.html" method="POST">
    <table id="greenTable" class='initialGreenTable'>
              <tr>
                  <th colspan="4" align="center"><strong><spring:message code="prof.common.Util.customerBal"></spring:message></strong></th>
              </tr>
              <tr>
              ...
              <tr>
                  <td ><label><spring:message code="prof.common.Util.customer"></spring:message></label></td>
                  <td>
                       <select name="customerName" id="customerName">
                        <option value="-1" selected="selected">--No Customer Filter--</option>        
                            <c:forEach items="${PARTY_LIST}" var="c" varStatus="count">
                                <option value='${c.id}' <c:if test="${custName == c.id}"> selected</c:if>>${c.companyName}</option>
                            </c:forEach>
                       </select>
                  </td>
              </tr>
	              <tr>
	                  ...
	                  <td><select name="branch" id="branch">
	                        <option value="-1" selected="selected">--No Branch--</option>        
       ...
       <div id="customerBalanceSheetData">
	       <table id="greenTable" width="100%">
	            ...
	            <c:if test="${custList!=null}">
		            <c:forEach var="a" items='${custList}' varStatus="count">
			            <c:if test="${a.custName != compName && count.index>0}">
			             ...
			             <c:if test="${a.id != partyAccId}">
		                         <c:forEach items="${SELF_LIST}" var="s" varStatus="cnt">
		                             <c:if test="${a.id == s.id && count.index==0}">
		                 ...
		                 <c:if test="${a.custName != compName}">
	                         ...
	                         <tr><td colspan="18">Buyer Name :-  <strong>${a.custName}</strong></td></tr>
